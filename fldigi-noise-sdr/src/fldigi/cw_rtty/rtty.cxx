...
void rtty::rx_init() {
  ...
  //// Synop file is reloaded each time we enter this modem. Ideally do that
...
void rtty::init() {
  ...
  // for (int i = 0; i < MAXPIPE; i++) mark_history[i] = space_history[i] =
  // cmplx(0,0);
...
void rtty::restart() {
  ...
  rtty_shift = shift =
      (progdefaults.rtty_shift < numshifts ? SHIFT[progdefaults.rtty_shift]
                                           : progdefaults.rtty_custom_shift);
  if (progdefaults.rtty_baud > numbauds - 1)
    progdefaults.rtty_baud = numbauds - 1;
  rtty_baud = BAUD[rtty_baud_idx];
  filter_length = FILTLEN[rtty_baud_idx];
  ...
  if (rtty_bits == 5)
    ...
    case 0:
      ...
      break;
    case 1:
      rtty_parity = RTTY_PARITY_EVEN;
      break;
    case 2:
      rtty_parity = RTTY_PARITY_ODD;
      break;
    case 3:
      rtty_parity = RTTY_PARITY_ZERO;
      break;
    case 4:
      rtty_parity = RTTY_PARITY_ONE;
      break;
    default:
      rtty_parity = RTTY_PARITY_NONE;
      break;
  ...
  for (int i = 0; i < MAXPIPE; i++)
    mark_history[i] = space_history[i] = cmplx(0, 0);
...
rtty::rtty(int _rtty_baud, bool _rtty_shaped) {
  rtty_baud_idx = _rtty_baud;
...
bool rtty::is_mark_space(int &correction) {
  ...
  if (bit_buf[0] && !bit_buf[symbollen - 1]) {
    ...
    if (abs(symbollen / 2 - correction) <
        6) // too small & bad signals are not decoded
      return true;
  }
  return false;
}

bool rtty::is_mark() { return bit_buf[symbollen / 2]; }
...
#define ook(sp)                                                                \
  {                                                                            \
    value = sin(2.0 * M_PI *                                                   \
                ((((sp / symbollen) % 2 == 0) ? (frequency + shift / 2.0)      \
                                              : (frequency - shift / 2.0)) /   \
                 samplerate) *                                                 \
                sp);                                                           \
  }
...
int rtty::rx_process(const short *buf, int len) {
  ...
  // ook_signal << ",,," << mnum++ + filter_length / 2 << "," << mark_mag << ","
  // << space_mag << "\n";
  //#endif
  ...
  //// Linear ATC
  ////			v1 = mark_mag - space_mag - 0.5 * (mark_env -
  /// space_env);
  //// Clipped ATC
  ////			v2  = (mclipped - noise_floor) - (sclipped - noise_floor) -
  ///0.5
  ///*
  ///(
  ////					(mark_env - noise_floor) - (space_env -
  /// noise_floor));
  //// Optimal ATC
  ...
  ////			v4 =  (mark_mag - noise_floor) * (mark_mag -
  /// noise_floor)
  ///-
  ////					(space_mag - noise_floor) * (space_mag -
  ///noise_floor)
  ///-
  /// 0.25 * (
  ////					(mark_env - noise_floor) * (mark_env -
  /// noise_floor)
  ///-
  ////					(space_env - noise_floor) * (space_env -
  /// noise_floor));
  //// Kahn Squarer with Clipped ATC
  ////			v5 =  (mclipped - noise_floor) * (mclipped -
  /// noise_floor)
  ///-
  ////					(sclipped - noise_floor) * (sclipped - noise_floor)
  ///-
  /// 0.25
  ///* (
  ////					(mark_env - noise_floor) * (mark_env -
...
void rtty::send_char(int c) {
  ...
  //	if (progdefaults.useFSK && (CW_KEYLINE_isopen ||
...
void send_fsk(int c)
{
        ...
        int stoplen = bitlen * (progdefaults.rtty_stop == 0 ? 1.0 :
progdefaults.rtty_stop == 1 ? 1.5 : 2.0);
...
void start_fsk_thread(void)
{
        ...
        if(pthread_mutex_init(&fsk_mutex, NULL)) {
                LOG_ERROR("AUDIO_ALERT thread create fail
(pthread_mutex_init)");
                return;
        }
...
SymbolShaper::~SymbolShaper() { delete[] m_sinc_table; }
