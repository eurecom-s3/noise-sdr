...
void contestia::send_tones() {
  if (!progdefaults.contestia_start_tones)
    return;
...
int contestia::tx_process() {
  ...
  if (tones != progdefaults.contestiatones || bw != progdefaults.contestiabw ||
...
// Android int contestia::rx_process(const double *buf, int len)
int contestia::rx_process(const short *buf, int len) {
  int c;
  unsigned char ch = 0;
  ...
  // Android Rx->SyncThreshold = progStatus.sqlonoff ?
  //	clamp(progStatus.sldrSquelchValue / 5.0 + 3.0, 3.0, 90.0) : 3.0;
  Rx->SyncThreshold = clamp(sldrSquelchValue / 5.0 + 3.0, 3.0, 90.0);

  Rx->Process(buf, len);
  /*Android
  sp = 0;
  for (int i = frequency - Rx->Bandwidth/2; i < frequency - 1 + Rx->Bandwidth/2;
  i++)
          if (wf->Pwr(i) > sp)
                  sp = wf->Pwr(i);
...
*/
  ...
  if (gotchar) {
              ...
              snprintf(msg2, sizeof(msg2), "f/o %+4.1f Hz",
  Rx->FrequencyOffset());
...
void contestia::restart() {
  ...
  if (Tx->Preset() < 0) {
    // Android LOG_ERROR("contestia: transmitter preset failed!");
    return;
  }
  ...
  if (Rx->Preset() < 0) {
    // Android LOG_ERROR("contestia: receiver preset failed!");
    return;
  }
  fragmentsize = 1024;
  set_bandwidth(Tx->Bandwidth - Tx->Bandwidth / Tx->Tones);

  /*Android	stringstream info;
...
// Androdi contestia::contestia(trx_mode omode)
contestia::contestia(int omode) {

  progdefaults.contestiasmargin = 8; // getPreferenceI("OLIVIASMARGIN", 8);
  progdefaults.contestiasinteg = 4;  // getPreferenceI("OLIVIASINTEG", 4);
  progdefaults.contestia_reset_fec =
      false; // getPreferenceB("OLIVIARESETFEC", false);
  // progdefaults.olivia8bit		  	= true;
  // //getPreferenceB("OLIVIA8BIT",
  // true);

  progdefaults.contestiasmargin =
      (int)clamp(progdefaults.oliviasmargin, 2.1, 128.1);
  progdefaults.contestiasinteg =
      (int)clamp(progdefaults.oliviasinteg, 2.1, 128.1);
  ...
  case MODE_CONTESTIA_8_500:
    progdefaults.contestiatones = tones = 2;
    progdefaults.contestiabw = bw = 2;
    // Android REQ(set_contestia_tab_widgets);
    break;
  ...
  for (int i = 0; i < SR4 / 8; i++)
    ampshape[i] = ampshape[SR4 - 1 - i] =
        0.5 * (1.0 - cos(M_PI * i / (SR4 / 8)));
