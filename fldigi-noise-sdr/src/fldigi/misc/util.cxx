...
/// Transforms the version, as a string, into an integer, so comparisons are
/// possible.
...
int set_cloexec(int fd, unsigned char v) {
  ...
  return f == -1 ? f
                 : fcntl(fd, F_SETFD, (v ? f | FD_CLOEXEC : f & ~FD_CLOEXEC));
}
#endif // __MINGW32__
...
//#ifndef __MINGW32__
  ...
  //return f == -1 ? f
                 //: fcntl(fd, F_SETFL, (v ? f | O_NONBLOCK : f & ~O_NONBLOCK));
//#else  // __MINGW32__
  //u_long v_ = (u_long)v;
  //errno = 0;
...
//int set_nodelay(int fd, unsigned char v) {
  //int val = v;
  //return setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, (const char *)&val,
                    //sizeof(val));
//}
...
/// Returns 0 if a process is running, 0 if not there and -1 if the test cannot
/// be made.
...
/// Starts a process and returns its pid, and -1 if error. Returns 0 if this
/// cannot be made.
int fork_process(const char *cmd) {
  ...
  if (!CreateProcess(NULL, cmd_local, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL,
                     NULL, &si, &pi))
