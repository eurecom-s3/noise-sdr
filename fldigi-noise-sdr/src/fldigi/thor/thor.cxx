...
void thor::reset_filters() {
  ...
  if (slowcpu) {
    // REVERTED FROM Android further reduce CPU load (at the detriment of some
    // decoding. Used
  ...
  // LOG_INFO("MAX ARRAY SIZE %d, paths %d, numbins %d, array_size %d",
  // MAXPATHS, paths, numbins, numbins * paths);
...
void thor::restart() { filter_reset = true; }
...
thor::thor(int md) : hilbert(0), fft(0), filter_reset(false) {
  ...
  progdefaults.ThorCWI = 0.0f;       // getPreferenceD("THORCWI", 0.0f);
  progdefaults.THOR_FILTER = true;   // getPreferenceB("THORFILTER", true);
  progdefaults.THOR_BW = 2.0f;       // getPreferenceD("THORBW", 2.0f);
  progdefaults.THOR_PREAMBLE = true; // getPreferenceB("THORPREAMBLE", true);
  progdefaults.THOR_SOFTSYMBOLS =
      true;                          // getPreferenceB("THORSOFTSYMBOLS", true);
  progdefaults.THOR_SOFTBITS = true; // getPreferenceB("THORSOFTBITS", true);
  ...
  // Android: add slowcpu impact on filter (taken from Java modem Thor.java in
  // AndPskmail)
  ...
  if (mode == MODE_THOR100 || mode == MODE_THOR50x1 || mode == MODE_THOR50x2 ||
    ...
    // Androis check: Fldigi 4.1.15:  Dec->settraceback (15 * 12); // Long
    // constraint length codes require longer traceback
    Dec->settraceback(
        PATHMEM - 1); // Long constraint length codes require longer traceback
  } else {
...
void thor::recvchar(int c) {
  ...
  //		put_sec_char(c & 0xFF);
    ...
    put_rx_char(c & 0xFF);
    // Android Not used here		if (progdefaults.Pskmails2nreport &&
    // (mailserver
    // || mailclient)) {
    //			if (((c & 0xFF) == SOH) && !s2n_valid) {
    //				// starts collecting s2n from first SOH in
    // stream
    //(since
    // start
    // of
...
void thor::decodePairs(unsigned char symbol) {
  ...
  if (met < 255 / 2)
    fec_confidence -= 2 + fec_confidence / 2;
  else
    fec_confidence += 2;
  if (fec_confidence < 0)
    fec_confidence = 0;
  if (fec_confidence > 100)
    fec_confidence = 100;
...
void thor::decodesymbol() {
  ...
  c = (int)floor(fdiff + .5);
  {
    if (progdefaults.THOR_PREAMBLE) {
      if (preambledetect(c)) {
        softflushrx(); // Flush the soft rx pipeline with punctures (interleaver
                       // & viterbi decoder)
        return;
      }
    }
...
void thor::softdecodesymbol() {
  ...
  // For a perfect & undistorted symbol, rawdoppler will == 0 (be a perfect
  // multiple of paths*doublespaced)
  ...
  if (0 == rawdoppler)
    ...
    // Detect modem "de-sync + immediate re-sync" events and reverse the
    // incurred soft-penalty
  ...
  // Android no squelch yet	if (progStatus.sqlonoff && metric <
  // progStatus.sldrSquelchValue) nowmag = 0;
...
#if SOFTPROFILE
  if (outofrange)
    LOG_INFO("%s", "outofrange");
  if (staticburst)
    LOG_INFO("%s", "staticburst");
...
int thor::harddecode() {
  ...
  for (int i = 0; i < paths * numbins; i++)
    // Android old complex class		avg +=
    // abs(pipe[pipeptr].vector[i]);
  ...
  for (int i = 0; i < paths * numbins; i++) {
    ...
    for (int j = 1; j <= numtests; j++) {
      ...
      if (p < 0)
        p += twosym;
      // Android old complex			cwmag =
      // abs(pipe[j].vector[i])/numtests;
...
int thor::softdecode() {
  ...
  static const int SoftBailout =
      6; // Max number of attempts to get a valid symbol
  ...
  // Allow for case where symbol == prev2symbol  (before calculating signal
  // average...)
  if (prev2symbol && (prev2symbol < MAXPATHS - 1)) // array bounds checking
    lastCWI[prev2symbol - 1] = lastCWI[prev2symbol] = lastCWI[prev2symbol + 1] =
        false;

  // Constrict the tone set further so CWI detect & set does not go out of
  // intended range
  lowest_tone += 1;
  highest_tone -= 1;
  ...
  } while (nextCWI[symbol] &&
           soft_symbol_trycount < SoftBailout); // Run while the detected symbol
                                                // has been identified as CWI
                                                // (alt: bailout after 6 trys)

  // Copy the newly-detected CWI mask to the static lastCWI array for use on
  // next function call
  for (int i = lowest_tone - 1; i < highest_tone + 1; i++)
    lastCWI[i] = nextCWI[i];

  staticburst = (max / avg < 1.2);

  // Return a NULL / Puncture symbol if a bailout occured
  if (soft_symbol_trycount >= SoftBailout)
    return 0;
  else
    return symbol;
}
...
void thor::softflushrx() {
  ...
  for (int i = 0; i < 90; i++)
    Rxinlv->symbols(flushsymbols);
  // flush viterbi with puncture soft-bits
  for (int j = 0; j < 128; j++)
    Dec->decode(puncture, NULL);
}
...
//LOG_INFO("%s", "cleared videodata");
        if (!progStatus.sqlonoff || metric >= progStatus.sldrSquelchValue) {
                ...
                for (int i = 0; i < paths * numbins; i++ ) {
                        if (range > 2) {
                                mag = (abs(pipe[pipeptr].vector[i]) - min) /
range + 0.0001;
                                mag = 1 + 2 * log10(mag);
                                if (mag < 0) mag = 0;
                        } else
        ...
        if (!progStatus.sqlonoff || metric >= progStatus.sldrSquelchValue) {
                for (unsigned int i = 0, j = 0; i < THORSCOPESIZE; i++) {
                        j = (pipeptr + i * twosym / THORSCOPESIZE + 1) %
(twosym);
                        scopedata[i] =
vidfilter[i]->run(abs(pipe[j].vector[prev1symbol]));
                }
        }
...
void thor::synchronize() {
  ...
  for (unsigned int i = 0, j = pipeptr; i < twosym; i++) {
    // Android old complex class		val =
    // abs(pipe[j].vector[prev1symbol]);
...
void thor::eval_s2n() {
  // Android old complex class	double s =
  // abs(pipe[pipeptr].vector[currsymbol]);
  double s = (pipe[pipeptr].vector[currsymbol]).mag();
  double n = (THORNUMTONES - 1) *
             //			abs( pipe[(pipeptr + symlen) %
  ...
  */
  ...
  int scalefec;
  if (fec_confidence++ > 90)
    scalefec = 100;
  else if (fec_confidence++ > 60)
    scalefec = 75;
  else if (fec_confidence++ > 40)
    scalefec = 50;
  else if (fec_confidence++ >= 20)
    scalefec = 25;
  else if (fec_confidence > 9)
    scalefec = 10;
  else
    scalefec = 0; // Else just round to 0.

  // Android not used here	snprintf(confidence, sizeof(confidence), "FEC:
  // %3.1d%%", scalefec);
  // Android	put_Status2(confidence);
}

// Android change to buffer of shorts int thor::rx_process(const double *buf,
// int len)
  ...
  while (len) {
    ...
    if (n) {
      for (int i = 0; i < n; i++) {
        ...
        // process THORMAXFFTS sets of sliding FFTs spaced at 1/THORMAXFFTS bin
        ...
        if (--synccounter <= 0) {
          ...
          // Android old complex class					currmag
          // =
          // abs(pipe_pipeptr_vector[currsymbol]);
