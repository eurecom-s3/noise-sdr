...
void fftfilt::init_filter() {
  flen2 = flen >> 1;
  // Android	fft			= new g_fft<double>(flen);
  fft = new Cfft(flen);
...
void fftfilt::create_filter(double f1, double f2) {
  ...
  for (int i = 0; i < flen2; i++) {
    ...
    // lowpass @ f2
    if (b_lowpass)
      ht[i] += fsinc(f2, i, flen2);
    // highighpass @ f1
    if (b_highpass)
      ht[i] -= fsinc(f1, i, flen2);
  }
  ...
  // Android (bouble check this substitution)	fft->ComplexFFT(filter);
  fft->cdft(filter);
  ...
  double scale = 0, mag;
  for (int i = 0; i < flen2; i++) {
    // Android old complex class		mag = abs(filter[i]);
    mag = (filter[i]).mag();
    if (mag > scale)
      scale = mag;
  }
  ...
  /*
          ...
          fspec <<
     "i,imp.re,imp.im,filt.re,filt.im,filt.abs,revimp.re,revimp.im\n";
          for (int i = 0; i < flen2; i++)
                  fspec
...
int fftfilt::run(const cmplx &in, cmplx **out) {
  ...
  if (inptr < flen2)
    return 0;
  if (pass)
    --pass; // filter output is not stable until 2 passes

  // FFT transpose to the frequency domain
  memcpy(freqdata, timedata, flen * sizeof(cmplx));
  // Android (bouble check this substitution)
  ...
  // Android double check this substitution
...
void fftfilt::rtty_filter(double f) {
  ...
  for (int i = 0; i < flen2; ++i) {
    ...
    dht = x <= 0 ? 1.0 : x > 2.0 * f ? 0.0 : cos((M_PI * x) / (f * 4.0));
    ...
    filter[i] = cmplx(dht * cos((double)i * -0.5 * M_PI),
                      dht * sin((double)i * -0.5 * M_PI));

    filter[(flen - i) % flen] = cmplx(dht * cos((double)i * +0.5 * M_PI),
                                      dht * sin((double)i * +0.5 * M_PI));
  }
