...
int olivia::tx_process() {
  ...
  if ((mode == MODE_OLIVIA &&
       (tones != progdefaults.oliviatones || bw != progdefaults.oliviabw)) ||
...
int olivia::rx_process(const short *buf, int len) {
  ...
  /*Android Not yet
          ...
          np = wf->Pwr(static_cast<int>(frequency + rx_bw/2 +
     2*rx_bw/rx_tones));
  ...
  */
  ...
  /*Android not here
          if (gotchar) {
                  snprintf(msg1, sizeof(msg1), "s/n: %4.1f dB", 10*log10(snr) -
     20);
...
void olivia::restart() {
  ...
  { // critical section
    ...
    if (Tx->Preset() < 0) {
      // Android not yet		LOG_ERROR("olivia: transmitter preset
      // failed!");
      return;
    }
    Tx->Start();
    ...
    //// Android
    ////	Rx->SyncThreshold = progStatus.sqlonoff ?
    ////		clamp(progStatus.sldrSquelchValue / 5.0 + 3.0, 0, 90.0)
    ///:
    /// 0.0;
    // Rx->SyncThreshold = clamp(sldrSquelchValue / 5.0 + 3.0, 0, 90.0);
    ...
    // if (Rx->Preset() < 0) {
    //// Android Not yet		LOG_ERROR("olivia: receiver preset
    //// failed!");
    // return;
    //}
    // fragmentsize = 1024;
    ...
    /*Android MUST BE FIXED IF USING PARAMETERS
            ...
            if (mode == MODE_OLIVIA)
                    put_MODEstatus("%s %" PRIuSZ "/%" PRIuSZ "",
       get_mode_name(), Tx->Tones, Tx->Bandwidth);
            else
                    put_MODEstatus("%s",
       mode_info[mode].sname);//get_mode_name());
    */
...
olivia::olivia(int omode) {
  ...
  progdefaults.oliviasmargin = 8; // getPreferenceI("OLIVIASMARGIN", 8);
  progdefaults.oliviasinteg = 4;  // getPreferenceI("OLIVIASINTEG", 4);
  progdefaults.olivia_reset_fec =
