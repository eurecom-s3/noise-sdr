...
unsigned char *xmtpicbuff = NULL;

// Android used elsewhere
mfsk *serviceme = 0;
...
void setpicture_link(mfsk *me) { serviceme = me; }
...
mfsk::~mfsk() {
  stopflag = true;
  int msecs = 200;
  while (--msecs && txstate != TX_STATE_PREAMBLE)
    MilliSleep(1);
...
void mfsk::recvpic(cmplx z) {
  ...
  if ((counter % RXspp) == 0) {
    ...
    if (pixelnbr % (picW * 3) == 0) {
      snprintf(mfskmsg, sizeof(mfskmsg), "Rx pic: %3.1f%%",
               (100.0f * pixelnbr) / n);
...
void mfsk::recvchar(int c) {
  ...
  /*Android Not used here
          ...
          if (progdefaults.Pskmails2nreport && (mailserver || mailclient)) {
                  if ((c == SOH) && !s2n_valid) {
                          // starts collecting s2n from first SOH in stream
     (since start of RX)
                          s2n_valid = true;
...
void mfsk::softdecode(cmplx *bins) {
  ...
  static const int CWI_MAXCOUNT = 6; // this is the maximum number of repeated
                                     // tones which is valid for the modem ( 0
                                     // excluded )
  ...
  // dynamic CWI avoidance: use harddecode() result (currsymbol) for CWI
  // detection
  if (reverse)
  ...
  for (i = 0; i < numtones; i++) {
    ...
    if (CWIcounter[k] > CWI_MAXCOUNT) {
      ...
      // Android		binmag = 2.0f * abs(bins[k]); // give
      // harddecode()
      // a vote in softdecode's decision.
      binmag =
          2.0f *
          bins[k].mag(); // give harddecode() a vote in softdecode's decision.
    else
  ...
  /*Android Not yet
          ...
          if (!progStatus.sqlonoff || metric >= progStatus.sldrSquelchValue)
                  for (unsigned int i = 0; i < SCOPESIZE; i++) {
                          j = (pipeptr + i * pipelen / SCOPESIZE + 1) %
     (pipelen);
                          scopedata[i] =
...
int mfsk::rx_process(const short *buf, int len) {
  ...
  while (len-- > 0) {
    ...
    // Android: made conditional to "SLOW CPU" as it is CPU intensive on older
    // Android devices
...
void mfsk::send_prologue() { flush_xmt_filter(tracepair.delay); }

void mfsk::send_epilogue() { flush_xmt_filter(64); }
  ...
  case TX_STATE_PICTURE:
    ...
    while (i < xmtbytes) {
      ...
      if ((100 * i / xmtbytes) % 2 == 0) {
        snprintf(mfskmsg, sizeof(mfskmsg), "Tx pic: %3.1f%%",
                 (100.0f * i) / xmtbytes);
