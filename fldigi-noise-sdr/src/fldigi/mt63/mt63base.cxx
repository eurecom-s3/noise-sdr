...
int MT63tx::Preset(double freq, int BandWidth, int LongInterleave) {
  ...
  TxAmpl = 4.0 / DataCarriers; // for maximum output level we can set
                               // TxAmpl=4.0/DataCarriers
  ...
  // blackman3 filter provides flat passband and sufficient out-of-band
  // rejection
  ...
  // compute dspPhase correction between successive FFTs separated by
  // SymbolSepar
...
// of the bit indices modulo FFT.size, i.e. r = FFT.BitRevIdx[c &  (FFT.Size -
// 1)]
...
int MT63tx::SendTune(bool twotones) {
  ...
  Ampl = TxAmpl * sqrt((double)(DataCarriers / 2));
...
int MT63encoder::Preset(int Carriers, int Intlv, int *Pattern, int PreFill) {
  ...
  if (IntlvLen) {
    if (dspRedspAllocArray(&IntlvPipe, IntlvSize))
      goto Error;
    if (PreFill)
    ...
    if (dspRedspAllocArray(&IntlvPatt, DataCarriers))
      goto Error;
    IntlvPtr = 0;
  }
  if (dspRedspAllocArray(&WalshBuff, DataCarriers))
    goto Error;
  if (dspRedspAllocArray(&Output, DataCarriers))
    goto Error;
...
int MT63encoder::Process(char code) // encode an ASCII character "code"
{
  ...
  if (code < DataCarriers)
    ...
    WalshBuff[code - DataCarriers] = (-1.0);
...
int MT63decoder::Preset(int Carriers, int Intlv, int *Pattern, int Margin,
                        int Integ) {
  int i, p;
  ...
  if (dspRedspAllocArray(&DecodeSnrMid, ScanLen))
    goto Error;
  if (dspRedspAllocArray(&DecodeSnrOut, ScanLen))
    goto Error;
...
int MT63rx::Preset(double freq, int BandWidth, int LongInterleave, int Integ,
  ...
  if (dspRedspAllocArray(&FFTbuff, WindowLen))
    goto Error;
  if (dspRedspAllocArray(&FFTbuff2, WindowLen))
    goto Error;
  ...
  err = TestOfs.Preset(-0.25 *
                       (2.0 * M_PI / WindowLen)); // for decoder tests only
  if (err)
    goto Error;
  ...
  ScanFirst =
      FirstDataCarr - ScanMargin * DataCarrSepar; // first FFT bin to scan
  if (ScanFirst < 0)
    ScanFirst += WindowLen;
  ScanLen = (DataCarriers + 2 * ScanMargin) *
            DataCarrSepar; // number of FFT bins to scan

  for (s = 0; s < SymbolDiv; s++) {
    if (dspRedspAllocArray(&SyncPipe[s], ScanLen))
      goto Error;
    dspClearArray(SyncPipe[s], ScanLen);
  }
  SyncPtr = 0;

  if (dspRedspAllocArray(&SyncPhCorr, ScanLen))
    goto Error;
  ...
  for (s = 0; s < SymbolDiv; s++) {
    if (dspRedspAllocArray(&CorrelAver[s], FitLen))
      goto Error;
  }

  if (dspRedspAllocArray(&SymbFit, FitLen))
    goto Error;

  if (dspRedspAllocArray(&SymbPipe, TrackPipeLen))
    goto Error;
  dspClearArray(SymbPipe, TrackPipeLen);
  if (dspRedspAllocArray(&FreqPipe, TrackPipeLen))
    goto Error;
  ...
  if (dspRedspAllocArray(&RefDataSlice, DataScanLen))
    goto Error;
  ...
  if (dspRedspAllocArray(&DataPwrMid, DataScanLen))
    goto Error;
  dspClearArray(DataPwrMid, DataScanLen);
  if (dspRedspAllocArray(&DataPwrOut, DataScanLen))
    goto Error;
  dspClearArray(DataPwrOut, DataScanLen);

  if (dspRedspAllocArray(&DataSqrMid, DataScanLen))
    goto Error;
  dspClearArray(DataSqrMid, DataScanLen);
  if (dspRedspAllocArray(&DataSqrOut, DataScanLen))
    goto Error;
  dspClearArray(DataSqrOut, DataScanLen);

  if (dspRedspAllocArray(&DataVect, DataScanLen))
    goto Error;

  if (dspRedspAllocArray(&DatadspPhase, DataScanLen))
    goto Error;
  if (dspRedspAllocArray(&DatadspPhase2, DataScanLen))
    goto Error;

  err = Decoder.Preset(DataCarriers, DataInterleave, InterleavePattern,
                       DataScanMargin, IntegLen);
  if (err)
    goto Error;
...
int MT63rx::Process(double_buff *Input) {
  ...
  while ((SyncProcPtr + WindowLen) < ProcLine.InpLen) {
    ...
    // printf("SyncSymbConf=%5.2f, SyncLock=%d, SyncProcPtr=%d, SyncPtr=%d,
    // SymbPtr=%d, SyncSymbShift=%5.1f, SyncFreqOfs=%5.2f =>",
    ...
    if (SyncPtr == SymbPtr) {
      ...
      s2 = s1 + SymbolSepar / 2;
      //	  printf(" Sample at %d,%d (SyncProcPtr-%d), time
      // diff.=%d\n",s1,s2,SyncProcPtr-s1,s1-DataProcPtr);
      DataProcess(ProcLine.InpPtr + s1, ProcLine.InpPtr + s2, SyncFreqOfs,
                  s1 - DataProcPtr);
      DataProcPtr = s1;
    }
    // printf("\n");
...
void MT63rx::SyncProcess(dspCmpx *Slice) {
  ...
  SyncPtr =
      (SyncPtr + 1) & (SymbolDiv - 1); // increment the correlators pointer
  ...
  if (SpectraDisplay) {
    for (i = 0,
        j = FirstDataCarr + (DataCarriers / 2) * DataCarrSepar - WindowLen / 2;
  ...
  for (i = 0; i < ScanLen; i++) {
    ...
    pI =
        PrevSlice[i].re * SyncPhCorr[i].re - PrevSlice[i].im * SyncPhCorr[i].im;
    pQ =
        PrevSlice[i].re * SyncPhCorr[i].im + PrevSlice[i].im * SyncPhCorr[i].re;
    Correl.re = dQ * pQ + dI * pI;
    Correl.im = dQ * pI - dI * pQ;
    dspLowPass2(&Correl, CorrelMid[SyncPtr] + i, CorrelOut[SyncPtr] + i, W1, W2,
                W5);
    PrevSlice[i].re = dI;
    PrevSlice[i].im = dQ;
  }
    ...
    */
    ...
    for (s = 0; s < SymbolDiv; s++) {
      ...
      for (k = 0; k < 2 * DataCarrSepar; k++)
        DoCorrelSum(CorrelNorm[s] + k, CorrelNorm[s2] + k + DataCarrSepar,
                    CorrelAver[s] + k);
    }
    ...
    if (P > 0.0) {
      ...
      if (SymbShift < 0)
        SymbShift += SymbolDiv;
      // for (i=j-1; i<=j+1; i++) printf("
      // [%+5.2f,%+5.2f]",SymbFit[i].re,SymbFit[i].im);
      // make first estimation of FreqOfs
      // printf(" -> [%+5.2f,%+5.2f] =>",I,Q);
      // for (i=j-2; i<=j+2; i++) printf("
      // %+6.3f",I*SymbFit[i].re+Q*SymbFit[i].im);
    ...
    if (SyncLocked) { // flip the SymbTime if it doesn't agree with the
                      // dspAverage
    ...
    //		Loops =
    dspSelFitAver(SymbPipe, TrackPipeLen, (double)3.0, 4, AverSymb, dspRMS,
                  Incl);
    // printf(" AverSymb=[%+5.2f,%+5.2f], dspRMS=%5.3f/%2d",
    //		 AverSymb.re,AverSymb.im,dspRMS,Incl);
    // find dspAverage freq. offset
    //		Loops =
    dspSelFitAver(FreqPipe, TrackPipeLen, (double)2.5, 4, AverFreq, dspRMS,
                  Incl);
    SyncFreqDev = dspRMS;
...
void MT63rx::DataProcess(dspCmpx *EvenSlice, dspCmpx *OddSlice, double FreqOfs,
                         int TimeDist) {
  int i, c, r;
  dspCmpx Freq, Phas;
  int incr, p;
  ...
  */
  ...
  incr =
      (TimeDist * DataCarrSepar) & WindowLenMask; // correct FFT dspPhase shift
  ...
  for (c = DataScanFirst, i = 0; i < DataScanLen;) { // TimeDist
    ...
    // printf("
    // %5.3f/%5.3f",dspAmpl(FFTbuff[c]),dspAmpl(FFTbuff[c+DataCarrSepar]));
    // printf("
    // %5.3f/%5.3f",dspAmpl(FFTbuff2[c+DataCarrSepar]),dspAmpl(FFTbuff2[c]));
    // printf("\n");
    Phas = FFT.Twiddle[p];
    ...
    dspLowPass2(dspPower(FFTbuff[c]), DataPwrMid[i], DataPwrOut[i], dW1, dW2,
                dW5);
    ...
    dspLowPass2(dspPower(FFTbuff2[c]), DataPwrMid[i], DataPwrOut[i], dW1, dW2,
                dW5);
    ...
    { // printf("%2d: [%8.5f,%8.5f]\n",i,DataVect[i].re,DataVect[i].im);
          ...
          printf("%2d: %6.3f [%6.3f,%6.3f]  [%8.5f,%8.5f], %5.2f, %5.2f",
             i, DataPwrOut[i], DataSqrOut[i].re,DataSqrOut[i].im,
                    DataVect[i].re,DataVect[i].im,
    DatadspPhase[i],DatadspPhase2[i]);
          if (DataPwrOut[i]>0.0)
