...
{
  return 0.35875 + 0.48829 * cos(dspPhase) + 0.14128 * cos(2 * dspPhase) +
         0.01168 * cos(3 * dspPhase);
}

// ----------------------------------------------------------------------------
// FIR shape calculation for a flat response from FreqLow to FreqUpp

void dspWinFirI(double LowOmega, double UppOmega, double *Shape, int Len,
                double (*Window)(double), double shift) {
  int i;
  ...
  for (i = 0; i < Len; i++) {
    ...
    // printf("%2d %5.1f %5.2f %7.4f
    // %7.4f\n",i,time,dspPhase,shape,(*Window)(dspPhase));
    Shape[i] = shape * (*Window)(dspPhase) / M_PI;
  }
}

void WinFirQ(double LowOmega, double UppOmega, double *Shape, int Len,
             double (*Window)(double), double shift) {
  int i;
  double time, dspPhase, shape;
...
int dspConvS16todouble(short int *dspS16, double_buff *dble, int Len,
                       double Gain) {
...
int dspConvU8todouble(unsigned char *U8, double_buff *dble, int Len,
                      double Gain) {
  int err = dble->EnsureSpace(Len);
  if (err)
    return -1;
...
void dspLowPass2(dspCmpx *Inp, dspCmpx *Mid, dspCmpx *Out, double W1, double W2,
                 double W5) {
  double Sum, Diff;
...
int dspPeriodLowPass2::Preset(int Period, double IntegLen) {
  int i;
  Len = Period;
  if (dspRedspAllocArray(&TapMid, Len))
    goto Error;
  if (dspRedspAllocArray(&TapOut, Len))
    goto Error;
  for (i = 0; i < Len; i++) {
    TapMid[i] = 0.0;
    TapOut[i] = 0.0;
...
int dspPeriodLowPass2::Process(double *Inp, int InpLen, double *Out) {
  int i, batch;
  for (i = 0; i < InpLen;) {
    for (batch = dspIntmin(InpLen - i, Len - TapPtr), i += batch; batch;
         batch--) {
      dspLowPass2(*Inp++, TapMid[TapPtr], TapOut[TapPtr], W1, W2, W5);
      (*Out++) = TapOut[TapPtr++];
    }
...
int dspBoxFilter::Preset(int BoxLen) {
  int i;
  if (dspRedspAllocArray(&Tap, BoxLen))
    return -1;
  for (i = 0; i < BoxLen; i++)
    Tap[i] = 0;
  Len = BoxLen;
...
int dspFirFilter::Preset(int FilterLen, double *FilterShape) {
  int i;
  if (dspRedspAllocArray(&Tap, FilterLen))
    return -1;
...
int dspFirFilter::ComputeShape(double LowOmega, double UppOmega,
  ...
  if (dspRedspAllocArray(&Shape, Len))
    return -1;
...
int dspQuadrSplit::Preset(int FilterLen, double *FilterShape_I,
                          double *FilterShape_Q, int DecimateRate) {
  Len = FilterLen;
  if (!ExternShape) {
    free(ShapeI);
    free(ShapeQ);
...
int dspQuadrSplit::ComputeShape(double LowOmega, double UppOmega,
  ...
  if (dspRedspAllocArray(&ShapeI, Len))
    return -1;
  if (dspRedspAllocArray(&ShapeQ, Len))
    return -1;
...
int dspQuadrComb::Preset(int FilterLen, double *FilterShape_I,
                         double *FilterShape_Q, int DecimateRate) {
  int i;
  Len = FilterLen;
  if (dspRedspAllocArray(&Tap, Len))
...
int dspRateConvQuadr::Process(double *Inp, int InpLen, double *Out,
  ...
  for (o = i = 0; (i < InpLen) && (o < MaxOutLen);) {
    if (OutdspPhase >= 1.0) {
      ...
      (*Out++) = Ref0 * (1.0 - OutdspPhase) + Ref1 * OutdspPhase // linear piece
                 -
                 (Diff1 - Diff0) * OutdspPhase * (1.0 - OutdspPhase) /
                     2; // quadr. piece
      o++;
      OutdspPhase += OutStep;
    }
...
int dspRateConvBL::Preset(int FilterLen, double **FilterShape,
                          int FilterShapeNum) {
  int i;
  Free();
  Len = FilterLen;
...
int dspRateConvBL::ComputeShape(double LowOmega, double UppOmega,
                                double (*Window)(double)) {
  int idx;
  if (ExternShape) {
    if (dspAllocArray(&Shape, ShapeNum))
      return -1;
    for (idx = 0; idx < ShapeNum; idx++) {
      if (dspAllocArray(&Shape[idx], Len))
        return -1;
    }
    ExternShape = 0;
  }
  for (idx = 0; idx < ShapeNum; idx++)
    dspWinFirI(LowOmega, UppOmega, Shape[idx], Len, Window,
               (double)idx / ShapeNum);
  return 0;
}
...
int dspCmpxSlideWindow::Preset(int WindowLen, int SlideDist,
                               double *WindowShape) {
  int i;
  ...
  Len = WindowLen;
  Dist = SlideDist;
  if (dspRedspAllocArray(&Buff, Len))
    return -1;
  for (i = 0; i < Len; i++)
...
int dspCmpxSlideWindow::SetWindow(double (*NewWindow)(double dspPhase),
                                  double Scale) {
  int idx;
  if (NewWindow == NULL) {
    if (!ExternWindow)
  ...
  if (dspRedspAllocArray(&Window, Len))
    return -1;
...
int dspCmpxSlideWindow::Process(dspCmpx_buff *Input) {
  ...
  while (InpLen > 0) {
    ...
    if (Ptr >= Len) {
      len = Output.Len;
      err = Output.EnsureSpace(len + Len);
      if (err)
        return err;
      if (Window == NULL)
...
int dspCmpxOverlapWindow::Preset(int WindowLen, int SlideDist,
                                 double *WindowShape) {
  int i;
  if (SlideDist > WindowLen)
    return -1;
...
int dspCmpxOverlapWindow::SetWindow(double (*NewWindow)(double dspPhase),
                                    double Scale) {
  int idx;
  if (NewWindow == NULL) {
    if (!ExternWindow)
...
int dspFFT_TimeShift::Preset(int FFTlen, int Backwards) {
  ...
  Len = FFTlen;
  LenMask = FFTlen - 1;
  if ((LenMask ^ Len) != (2 * Len - 1))
    return -1;
  if (dspRedspAllocArray(&FreqTable, Len))
    return -1;
  for (i = 0; i < Len; i++) {
    p = (2 * M_PI * i) / Len;
    if (Backwards)
...
void dspDiffBitSync4::Free() {
  free(InpTap);
  InpTap = NULL;
}

int dspDiffBitSync4::Process(double *Inp, int InpLen, double *BitOut,
                             double *IbitOut, int MaxOutLen, int *OutLen) {
  int i, o, t, step;
  double diff;
  ...
  for (step = 0, o = i = 0; (i < InpLen) && (o < MaxOutLen); i++) {
    ...
    dspLowPass2(diff * diff, DiffInteg0[DiffTapPtr], DiffInteg[DiffTapPtr], W1,
                W2, W5);
    if (DiffTapPtr == BitPtr) {
      ...
      if ((Sum == 0.0) ||
          ((SyncConfid = (SumI * SumI + SumQ * SumQ) / (Sum * Sum)) == 0.0)) {
        (*BitOut++) = 0;
        (*IbitOut++) = 0;
        o++;
        continue;
      ...
      bit =
          Samp[2] * (1.0 + dx) - Samp[1] * dx // or quadratic
          +
          ((Samp[3] - Samp[1]) - (Samp[2] - Samp[0])) / 2 * dx * (1.0 + dx) / 2;
      ibit =
          Samp[4] + dx * (Samp[4] - Samp[3]); // linear interpolation is enough
      (*BitOut++) = bit;
      (*IbitOut++) = ibit;
      o++;
...
dspBitSlicer::~dspBitSlicer() { free(Tap); }

int dspBitSlicer::Process(double *Bits, double *IBits, int InpLen,
                          double *OutBits) {
  int i, l;
  double Bit, soft;
  for (i = 0; i < InpLen; i++) {
    ...
    if (Noise[0] + Noise[1] <= 0)
      ...
      OptimThres =
          (Sum[0] * Noise[1] + Sum[1] * Noise[0]) / (Noise[0] + Noise[1]);
...
double dspBitSlicer::GetdspAmplAsym() {
  double Sweep = Sum[1] - Sum[0];
  return Sweep > 0 ? 2 * dspAmplAsym / Sweep : 0.0;
}

double dspBitSlicer::GetTimeAsym() {
  double Sweep = Sum[1] - Sum[0];
  return Sweep > 0 ? 2 * TimeAsym / Sweep : 0.0;
}
...
dspHDLCdecoder::~dspHDLCdecoder() { free(Buff); }
...
short unsigned int dspAX25CRCtable[256] = {
    0U,     4489U,  8978U,  12955U, 17956U, 22445U, 25910U, 29887U, 35912U,
    40385U, 44890U, 48851U, 51820U, 56293U, 59774U, 63735U, 4225U,  264U,
    13203U, 8730U,  22181U, 18220U, 30135U, 25662U, 40137U, 36160U, 49115U,
    44626U, 56045U, 52068U, 63999U, 59510U, 8450U,  12427U, 528U,   5017U,
    26406U, 30383U, 17460U, 21949U, 44362U, 48323U, 36440U, 40913U, 60270U,
    64231U, 51324U, 55797U, 12675U, 8202U,  4753U,  792U,   30631U, 26158U,
    21685U, 17724U, 48587U, 44098U, 40665U, 36688U, 64495U, 60006U, 55549U,
    51572U, 16900U, 21389U, 24854U, 28831U, 1056U,  5545U,  10034U, 14011U,
    52812U, 57285U, 60766U, 64727U, 34920U, 39393U, 43898U, 47859U, 21125U,
    17164U, 29079U, 24606U, 5281U,  1320U,  14259U, 9786U,  57037U, 53060U,
    64991U, 60502U, 39145U, 35168U, 48123U, 43634U, 25350U, 29327U, 16404U,
    20893U, 9506U,  13483U, 1584U,  6073U,  61262U, 65223U, 52316U, 56789U,
    43370U, 47331U, 35448U, 39921U, 29575U, 25102U, 20629U, 16668U, 13731U,
    9258U,  5809U,  1848U,  65487U, 60998U, 56541U, 52564U, 47595U, 43106U,
    39673U, 35696U, 33800U, 38273U, 42778U, 46739U, 49708U, 54181U, 57662U,
    61623U, 2112U,  6601U,  11090U, 15067U, 20068U, 24557U, 28022U, 31999U,
    38025U, 34048U, 47003U, 42514U, 53933U, 49956U, 61887U, 57398U, 6337U,
    2376U,  15315U, 10842U, 24293U, 20332U, 32247U, 27774U, 42250U, 46211U,
    34328U, 38801U, 58158U, 62119U, 49212U, 53685U, 10562U, 14539U, 2640U,
    7129U,  28518U, 32495U, 19572U, 24061U, 46475U, 41986U, 38553U, 34576U,
    62383U, 57894U, 53437U, 49460U, 14787U, 10314U, 6865U,  2904U,  32743U,
    28270U, 23797U, 19836U, 50700U, 55173U, 58654U, 62615U, 32808U, 37281U,
    41786U, 45747U, 19012U, 23501U, 26966U, 30943U, 3168U,  7657U,  12146U,
    16123U, 54925U, 50948U, 62879U, 58390U, 37033U, 33056U, 46011U, 41522U,
    23237U, 19276U, 31191U, 26718U, 7393U,  3432U,  16371U, 11898U, 59150U,
    63111U, 50204U, 54677U, 41258U, 45219U, 33336U, 37809U, 27462U, 31439U,
    18516U, 23005U, 11618U, 15595U, 3696U,  8185U,  63375U, 58886U, 54429U,
    50452U, 45483U, 40994U, 37561U, 33584U, 31687U, 27214U, 22741U, 18780U,
    15843U, 11370U, 7921U,  3960U};
...
inline void dsp_r2FFT::FFT4(dspCmpx &x0, dspCmpx &x1, dspCmpx &x2,
                            dspCmpx &x3) {
  dspCmpx x1W;
  x1W.re = x2.re;
  x1W.im = x2.im;
...
void dsp_r2FFT::CoreProc(dspCmpx x[]) {
  ...
  for (Groups = HalfSize / 2, GroupHalfSize = 2; Groups;
...
int dspSlideWinFFT::SetWindow(double (*NewWindow)(double dspPhase),
                              double Scale) {
  int idx, err;
  if (NewWindow == NULL) {
    if (!ExternWindow)
  ...
  for (idx = 0; idx < Size; idx++)
    Window[idx] =
        Scale * (*NewWindow)(2 * M_PI * (idx - Size / 2 + 0.5) / Size);
  return 0;
}
  ...
  while (InpLen) {
    ...
    if (Left == 0) {
      ...
      if (Slide) {
        ...
        if (err)
          return -1;
        FFT.SeparTwoReals(FFTbuff, Output.Data + len,
                          Output.Data + len + Size / 2);
        Output.Len += Size;
      }
    }
  }
...
int dspSlideWinFFTproc::Preset(int size, int step,
                               void (*proc)(dspCmpx *Spectra, int Len),
                               double (*NewWindow)(double dspPhase),
                               double Scale) {
  int err;
...
int dspSlideWinFFTproc::SetWindow(double (*NewWindow)(double dspPhase),
                                  double Scale) {
  int idx, err;
  if (NewWindow == NULL) {
    if (!ExternWindow)
