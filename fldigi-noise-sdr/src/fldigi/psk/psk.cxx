...
void psk::rx_init() {
  ...
  bitclk = 0;
  freqerr = 0.0;
  // Android 	if (mailserver && progdefaults.PSKmailSweetSpot) sigsearch =
  // SIGSEARCH;
...
psk::psk(int pskmode) : modem() {
  ...
  progdefaults.pskpilot = true;      // getPreferenceB("8PSKPILOT", true);
  progdefaults.pilot_power = -30.0f; // getPreferenceD("8PSKPILOTPOWER",
                                     // -30.0f);
  // Limit pilot_power
  if (progdefaults.pilot_power > -20.0f)
    progdefaults.pilot_power = -20.0f;
  if (progdefaults.pilot_power < -60.0f)
    progdefaults.pilot_power = -60.0f;
  ...
  */
  ...
  case MODE_8PSK1200F: // 1200 baud | 2400 bits/sec @ 2/3 rate FEC
    ...
    PSKviterbi = true;
    break;
  case MODE_8PSK500FX2: // 500 baud | 2000 bits/sec @ 2/3 OR 1500 bits/sec @ 1/2
                        // rate FEC
                        // Android			symbollen = 32;
    symbollen = 16;
    idepth = 640; // 426 milliseconds
  ...
  default:
    ...
    if (progdefaults.slowcpu) {
      ...
      for (int i = 0; i < numcarriers; i++) {
        fir1[i]->init(FIRLEN / 2, symbollen > 15 ? symbollen / 16 : 1, fir1c,
                      fir1c);
      ...
      for (int i = 0; i < numcarriers; i++) {
        fir1[i]->init(FIRLEN, symbollen > 15 ? symbollen / 16 : 1, fir1c,
                      fir1c);
        fir2[i]->init(FIRLEN, 1, fir2c, fir2c);
      }
    }
...
void psk::rx_bit(int bit) {
  ...
  /* Android not needed
          if (do_s2nreport) {
                  if (progdefaults.Pskmails2nreport && (mailserver ||
     mailclient)) {
                          s2n_sum += s2n_metric;
...
void psk::vestigial_afc() {
  ...
  if (!progdefaults.pskpilot)
    return;
  if (!vestigial_sfft->is_stable())
    return;
  ...
  for (i = 0; i < 11; i++)
    avg += sqrt(sfft_bins[i].norm());
  avg /= 11.0;
  std::setprecision(2);
  std::setw(5);
  // Android old complex.h    	for (i = 0; i < 11; i++) if (abs(sfft_bins[i])
  // > 2.0*avg) break;
  for (i = 0; i < 11; i++)
    if (sqrt(sfft_bins[i].norm()) > 2.0 * avg)
      break;
  if (i < 11) {
    //		std::cout	<< "bin: " << i
    //					<< ", freq offset: " << (i -
    // 5)*samplerate/16384.0
    //					<< ", amp: " << abs(sfft_bins[i])
    //					<< ", avg: " << avg << "\n";
    if (i != 5) {
...
void psk::phaseafc() {
  ...
  if (afcmetric < 0.05 || mode == MODE_PSK500 ||
      // Android		mode == MODE_QPSK500 ||
      numcarriers > 1)
    return;
  // Android: re-enable (delete return below and check for multi-carrier modes)
  // when have selectable afc
  return;
...
void psk::rx_symbol(cmplx symbol, int car) {
  ...
  // Android using older complex.h header	phase = arg (
  // conj(prevsymbol[car])
  // * symbol );
  ...
  //		metric *= 2 * symbits; /// @TODO scale the metric with the psk
  ...
  // Android old complex.h	afcmetric = decayavg(afcmetric, norm(quality),
  // 50);
  ...
  int set_dcdON =
      -1; // 1 sets DCD on ; 0 sets DCD off ; -1 does neither (no-op)
  imdValid = false;
  // printf("\n%.8X", dcdshreg);
  switch (dcdshreg) {
  ...
  default:
    // Android		if (metric > progStatus.sldrSquelchValue ||
    // progStatus.sqlonoff
    // == false) {
  ...
  if (1 == set_dcdON) {
    ...
    imdValid = true;
    // Android		if (progdefaults.Pskmails2nreport && (mailserver ||
    // mailclient))
  ...
  if (_pskr) {
    ...
    if (!_disablefec && (_16psk || _8psk || _xpsk)) {
      ...
      // printf("\n");
      if ((_puncturing && _16psk))
        rx_pskr(128); // 16psk: recover punctured low bit
      ...
      if (_8psk) {
        ...
        phasequality = (phasequality + lastphasequality) /
                       2; // Differential modem: average probabilities between
                          // current and previous symbols
        lastphasequality = phasequality;
        int soft_qualityerror = static_cast<int>(128 - (128 * phasequality));

        if (soft_qualityerror >
            255 - 25) // Prevent soft-bit wrap-around (crossing of value 128)
          softpuncture = true;
        else if (soft_qualityerror < 128 / 3) // First 1/3 of phase delta is
                                              // considered a perfect signal
          soft_qualityerror = 0;
        else if (soft_qualityerror >
                 128 - (128 / 8)) // Last 1/8 of phase delta triggers a puncture
          softpuncture = true;
        else
          soft_qualityerror /=
              2; // Scale the FEC error to prevent premature cutoff
        ...
        if (softpuncture) {
          ...
          for (int i = symbits - 1; i >= 0;
               i--) { // Use predefined Gray-mapped softbits for soft-decoding
            if (graymapped_8psk_softbits[bitindex][i] > 128) // Soft-One
              rx_pskr((graymapped_8psk_softbits[bitindex][i]) -
                      soft_qualityerror);
            else // Soft-Zero
              rx_pskr((graymapped_8psk_softbits[bitindex][i]) +
                      soft_qualityerror);
        ...
        // Hard Decode Section
        for (int i = 0; i < symbits;
             i++) { // Hard decode symbits into soft-symbols
          xsoftsymbols[i] = (bits & bitmask) ? 255 : 0;
          // printf(" %.3u ", xsoftsymbols[i]);
          rx_pskr(xsoftsymbols[i]); // Feed to the PSKR FEC decoder, one bit at
                                    // a time.
          bitmask = bitmask << 1;
        }
      }
      if (_puncturing)
        rx_pskr(128); // x/8/16psk: Recover punctured high bit
...
*/
  ...
  while (len-- > 0) {
    ...
    for (int car = 0; car < numcarriers; car++) {
      ...
      // Android from double to shorts			z = cmplx ( *buf *
      // cos(phaseacc[car]), *buf * sin(phaseacc[car]) );
      z = cmplx((double)(*buf) * cos(phaseacc[car]),
                (double)(*buf) * sin(phaseacc[car]));
      ...
      if (fir1[car]->run(z, z)) {   // fir1 returns true every Nth sample
        ...
        if (car == numcarriers - 1) {
          ...
          * waveform in the syncbuf array, according to its amplitude (not
          * phase).
          */
          ...
          for (int ii = 0; ii < numcarriers; ii++) {
            // Android old complex.h				sum +=
            // abs(z2[ii])/numcarriers;
            sum += sqrt(z2[ii].norm()) / numcarriers;
          }
          //			syncbuf[idx] = 0.8 * syncbuf[idx] + 0.2 *
          // z2[car].mag();
          ...
          * lower half of the array with its counterpart on the upper half of
          * the
          * array, or the other side of the waveform.  Each pair's difference is
          * divided by their sum, scaling it so that the signal amplitude does
          * not
          ...
          */
          ...
          /**
          * If the lower side is larger (meaning that the waveform is shifted in
          * that
          * direction), then the sum is negative, and bitclk needs to be
          * adjusted to
          * be a little faster, so that the next drawing of the waveform in
          * syncbuf
          * will be shifted right. Conversely, if the sum is positive, then it
          * needs
          ...
          */
          ...
          /**
          * When bitclock reaches the end of the buffer, then a complete
          * waveform
          ...
          *     0                              N/2 N-1
          *
          * A	t the end of this cycle, bitclk is pointing at a sample which
          * will
          * have the maximum phase difference, if any, from the previous
          * symbol's
          * phase.
          *
          */
...
void psk::tx_carriers() {
  ...
  frequencies[0] =
      get_txfreq_woffset() + ((-1 * numcarriers) + 1) * inter_carrier / 2;
  delta[0] = TWOPI * frequencies[0] / samplerate;
  ...
  for (int car = 0; car < symbols; car++) {
    ...
    if (_8psk && !_disablefec) // Use Gray-mapped 8psk constellation
      symbol = prevsymbol[car] *
               graymapped_8psk_pos[(sym & 7)]; // complex multiplication
      ...
      symbol = prevsymbol[car] *
               sym_vec_pos[(sym & SVP_MASK)]; // complex multiplication
    }
    ...
    for (int i = 0; i < symbollen; i++) {
      ...
      if (car != 0) {
        outbuf[i] += (ival * cos(phaseacc[car]) + qval * sin(phaseacc[car])) /
                     numcarriers;
      } else {
        outbuf[i] = (ival * cos(phaseacc[car]) + qval * sin(phaseacc[car])) /
                    numcarriers;
...
void psk::tx_xpsk(int bit) {
  ...
  /*
   else if (mode == MODE_8PSK ???) { // Puncture @ 5/6 rate | tx 3bits/symbol
   (8psk)
   ...
   */
    ...
    if (0 == bitcount) {
      ...
      /// Puncture -> //xpsk_sym |= (static_cast<unsigned int>(fecbits) & 2) <<
      /// 2 ;
    ...
    } else if (1 == bitcount) { // xpsk_sym buffer with one bit: add 2 bits then
                                // tx and clear
    ...
    } else if (2 == bitcount) { // xpsk_sym buffer with 2 bits: add 1 then tx
                                // and save next bit
                                // printf("\nxpsk_sym|preadd: %u", xpsk_sym);
      xpsk_sym |= (static_cast<unsigned int>(fecbits) & 1) << 2;
      // printf("\nxpsk_sym|postadd: %u", xpsk_sym);
...
void psk::tx_flush() {
  if (_pskr) {
    ...
    // VK2ETA replace with a more effective flushing sequence (avoids cutting
    // the last characters in low s/n)
    for (int i = 0; i < ovhd_symbols / 2; i++)
      tx_bit(0);
    ...
    for (int i = 0; i < symbits; i++) {
      tx_char(
          0); // Send <NUL> to clear bit accumulators on both Tx and Rx ends.
    }
    int symbol;
    if (_16psk)
      symbol = 8;
...
int psk::tx_process() {
  ...
  if (c == GET_TX_CHAR_ETX || stopflag) {
    ...
    ovhd_samples =
        (acc_symbols - char_symbols - ovhd_symbols) * symbollen / numcarriers;
...
void psk::calcSN_IMD(cmplx z) {
  ...
  if (++m_NCount >= GOERTZEL) {
    m_NCount = 0;
    for (i = 0; i < NUM_FILTERS; i++) {
      m_Energy[i] = I1[i] * I1[i] + Q1[i] * Q1[i] + I2[i] * I2[i] +
                    Q2[i] * Q2[i] - I1[i] * I2[i] * COEF[i] -
                    Q1[i] * Q2[i] * COEF[i];
      I1[i] = I2[i] = Q1[i] = Q2[i] = 0.0;
    }
    signalquality();
