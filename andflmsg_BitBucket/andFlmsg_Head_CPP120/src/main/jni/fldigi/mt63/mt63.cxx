...
//		if (progStatus.sqlonoff && snr < progStatus.sldrSquelchValue) {
		...
		for (i = 0; i < Rx->Output.Len; i++) {
			...
			//Android: this is why the lower control characters are never sent. Reason????
			if ((c < 8) && (escape == 0))
				continue;
			if (c == 127) {
				escape = 1;
...
mt63::mt63 (int mt63_mode) : modem()
{
	...
	progdefaults.mt63_rx_integration= getPreferenceB("MT63INTEGRATION", true);
	progdefaults.mt63_usetones	  	= getPreferenceB("MT63USETONES", true);
